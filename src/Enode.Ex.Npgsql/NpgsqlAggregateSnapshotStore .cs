...
{
    public class NpgsqlAggregateSnapshotStore : IAggregateSnapshotStore
    {
        ...
        HashSet<string> AggregateRootTypeSet = new HashSet<string>();
        ...
        public void Initialize(string connectionString,
            ...
            int tableCount = 1,
            string[]  aggregateTypeNames = null
            )
        {
            ...
            if(aggregateTypeNames !=null)
            {
                foreach (var item in aggregateTypeNames)
                {
                    AggregateRootTypeSet.Add(item);
                }
            }
        }
        ...
        public async Task<AggregateSnapshotHeader> FindLatestHeaderAsync(string aggregateRootId, string aggregateRootTypeName)
        {
            ...
            string SQL = $"SELECT \"AggregateRootId\",\"AggregateRootTypeName\",\"Version\""
        ...
        {
            if(! AggregateRootTypeSet.Contains(aggregateRootTypeName))
            {
                return null;
            }
            
            string SQL = $"SELECT \"AggregateRootId\",\"AggregateRootTypeName\",\"Version\",\"Data\""
                        +$"FROM {GetTableName(aggregateRootId)}"
                        +$"WHERE \"AggregateRootId\"=@AggregateRootId";
            try
            {
        ...
        {
            Console.WriteLine($"快照生产");
           
           const string SQL_FORMAT = "INSERT INTO {0} (\"AggregateRootId\",\"AggregateRootTypeName\",\"Version\",\"Data\")"+
                                     "VALUES (@AggregateRootId,@AggregateRootTypeName,@Version,@Data)"+
                                     "ON CONFLICT (\"AggregateRootId\")"+
